...
@Component({
    ...
    <div class="atm-screen-content">
      ...
      <div class="deposit-layout">
        <div class="quick-amounts">
          <button type="button" class="side-btn" (click)="amount = 20">20 ₺ ◀</button>
          <button type="button" class="side-btn" (click)="amount = 50">50 ₺ ◀</button>
          <button type="button" class="side-btn" (click)="amount = 100">100 ₺ ◀</button>
        </div>

        <div class="deposit-center">
          <div class="deposit-slot-visual">
            <div class="slot-light"></div>
            <span class="slot-text">BANKNOT HAZNESİ</span>
          </div>
          
          <div class="input-container">
            <label>YATIRILACAK TUTAR</label>
            <input type="number" [(ngModel)]="amount" placeholder="0" min="10" (keyup.enter)="deposit()">
          </div>
        </div>

        <div class="quick-amounts right">
          <button type="button" class="side-btn text-right" (click)="amount = 200">▶ 200 ₺</button>
          <button type="button" class="side-btn text-right" (click)="amount = 500">▶ 500 ₺</button>
          <button type="button" class="side-btn text-right" (click)="amount = 1000">▶ 1000 ₺</button>
        </div>
      </div>

      <div class="screen-footer-combined">
        <button class="back-btn-alt" (click)="goBack()">◀ GERİ DÖN</button>
        <button class="atm-action-btn" [disabled]="!amount || amount < 10 || isLoading" (click)="deposit()">
          {{ isLoading ? 'İŞLEM YAPILIYOR...' : 'ONAYLA' }}
        </button>
      </div>
    </div>
  `,
  styles: [`
    ...
    .screen-header h1 { font-size: 2rem; margin: 0; color: #2ecc71; }

    .deposit-layout { flex: 1; display: flex; justify-content: space-between; align-items: center; padding: 1rem 0; }
    
    .quick-amounts { display: flex; flex-direction: column; gap: 1.5rem; }
    .side-btn {
      background: #2c3e50; color: #fff; border: 2px solid #2ecc71; padding: 1rem 1.2rem;
      font-size: 1.1rem; font-weight: 900; cursor: pointer; min-width: 140px;
    }
    .side-btn:hover { background: #2ecc71; color: #000; }
    .text-right { text-align: right; }

    .deposit-center { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 2rem; }

    .deposit-slot-visual {
      width: 200px; height: 60px; background: #1a1a1a; border: 3px solid #333;
      border-radius: 8px; display: flex; flex-direction: column; align-items: center;
      justify-content: center; position: relative;
    }
    .slot-light {
      width: 150px; height: 3px; background: #2ecc71; border-radius: 2px;
      box-shadow: 0 0 10px #2ecc71; margin-bottom: 4px; animation: pulse 1.5s infinite;
    }
    .slot-text { font-size: 0.6rem; color: #666; font-weight: bold; }
    @keyframes pulse { 0% { opacity: 0.4; } 50% { opacity: 1; } 100% { opacity: 0.4; } }

    .input-container label { display: block; color: #2ecc71; margin-bottom: 0.5rem; font-size: 0.9rem; font-weight: bold; }
    .input-container input {
      background: #000; border: 2px solid #333; padding: 1rem; color: #fff;
      font-size: 2rem; width: 180px; text-align: center; outline: none; border-radius: 8px;
    }
...
export class DepositComponent {
  ...
  deposit() {
    ...
    this.atmService.deposit(this.amount).subscribe({
      next: (result) => {
        if (result.success) {
          this.playMoneySound();
          this.notificationService.show(`${this.amount} TL başarıyla yatırıldı. Paranız sayılıyor...`, 'success');
  ...
  playMoneySound() {
    try {
      ...
      // Create white noise buffer for paper rustling
